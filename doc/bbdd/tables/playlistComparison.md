...
| `youtube_playlist_state_fingerprint` | texto | no | - | Huella estable del estado persistido de `youtube_playlist_item` comparado |
| `local_library_state_fingerprint` | texto | no | - | Huella estable del estado persistido de `local_song` comparado |
...
* Para acelerar esa lectura existe un indice compuesto por scope y orden temporal:
  * `youtube_playlist_id`
  * `local_folder_id`
  * `compared_at`
  * `id`
